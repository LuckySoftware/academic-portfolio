...
const PortfolioVolunteering = ({ experiences = [] }) => { return (
        ...
        <div className="container mx-auto px-6 max-w-5xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                {experiences.map((exp, index) => (
                    <VolunteeringCard key={index} organization={exp.organization} role={exp.role} dates={exp.dates} description={exp.description} />
                ))}
            </div>
        </div>
    </section>
